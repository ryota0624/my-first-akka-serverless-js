...
curl -XPOST -H "Content-Type: application/json" -d '{"counterId": "foo"}' localhost:9000/com.example.CounterService/GetCurrentCounter

curl -v -XPOST -H "Content-Type: application/json" -d '{"cartId": "foo", "product_id": "huga", "name": "hoge", "quantity":  10}' localhost:9000/cart/10/items/add